...
void update(int k, int v) {
    for (int i = k; i < N; i += i&-i) cnt[i] += v;
}
int query(int k) {
    int ret = 0;
    for (int i = k; i; i -= i&-i) ret += cnt[i];
    return ret;
}
...
int main() {
    ...
    Rep(i,1,n) {
        ...
        for (auto maxi : maxs) {
            while (L[maxi] || R[maxi]) {
                if (L[maxi] && a[maxi] > a[L[maxi]]) {
                    printf("%d L\n", maxi-query(maxi));
                    update(L[maxi], 1);
                    a[maxi] += a[L[maxi]];
                    del(L[maxi]);
                }
                else if (R[maxi] && a[maxi] > a[R[maxi]]) {
                    printf("%d R\n", maxi-query(maxi));
                    update(R[maxi], 1);
                    a[maxi] += a[R[maxi]];
                    del(R[maxi]);
