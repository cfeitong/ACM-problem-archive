...
int check(LD l, LD h, LD t) {
    int ans = 0;
    if ( l >= 125 && h >= 90 && t >= 0.25 ) {
        if ( l <= 290 && h <= 155 && t <= 7 ) ans = 1;
        else if ( l <= 380 && h <= 300 && t <= 50 ) ans = 2;
        else if ( l + 2*(h+t) <= 2100 ) ans = 3;
    }
    return ans;
}
...
int main () {
    ...
    while ( in(l,h,t),
        ...
        if (h < t) swap(h, t);
        ans = check(l,h,t);
        const char res[4][100] = {"not mailable", "letter", "packet", "parcel"};
        out(res[ans],'\n');
    }
    return 0;
}
