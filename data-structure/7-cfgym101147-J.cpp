...
void dfs(int cur, int pa, int dep) {
    tr[dep] = cur;
    int l = -1, r = dep;
    while (l < r) {
        int m = (l + r + 1) / 2;
        if (X[cur] >= len[cur] - len[tr[m]]) {
            r = m - 1;
        } else {
            l = m;
        }
    ...
    for (auto& it : to[cur]) {
        if (it.first == pa) continue;
        len[it.first] = len[cur] + it.second;
        dfs(it.first, cur, dep + 1);
        ans[cur] += ans[it.first];
    }
}
